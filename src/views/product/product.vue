    ...
    <el-table :data="list" style="width: 100%">
        ...
        <el-table-column label="操作" align="center" width="200" #default="scope">
            ...
            <el-button v-if="scope.row.auditStatus == 0 || scope.row.auditStatus == -1" type="success" size="small"
                @click="audit(scope.row)" style="padding: 5px; margin-left: 5px;">
                审批
            </el-button>
            <el-button v-if="scope.row.auditStatus == 1 && (scope.row.status == 0 || scope.row.status == -1)
            " type="warning" size="small" @click="updateStatus(scope.row.id, 1)"
...
const audit = async row => {
    ...
    product.value = data
    product.value.brandName = row.brandName
    product.value.category1Name = row.category1Name
    product.value.category2Name = row.category2Name
    product.value.category3Name = row.category3Name
